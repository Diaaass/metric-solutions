...
- Минимальное время заполнения: клиент кладёт в скрытое поле метку времени показа формы,
  сервер отбрасывает отправки быстрее 3 секунд. Ответ тоже `200` — боту не сообщаем,
  на чём он попался. Проверка не хранит состояние и потому работает на serverless.
